    ...
    try:
        ...
        if result['success']:
            ...
            if df is not None and not df.empty:
                ...
                # Logic: จัด Format ให้พร้อมสำหรับ Visualization ทันที
                # Column แรกเป็น X, Column ที่สองเป็น Y (ถ้ามี)
                response = {
                    # "success": True,
                    "x_axis": df[cols[0]].tolist(),
                    "y_axis": df[cols[1]].tolist() if len(cols) > 1 else df[cols[0]].tolist(),
                    "x_label": cols[0],
        ...
        return json.dumps({
            'success': False,
            'error': f'Tool unexpected error: {str(e)}'
        })
